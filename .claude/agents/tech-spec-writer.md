...
3. **기능 목록 (Feature List)** — every distinct feature in scope, each with a short stable ID (e.g. `F1`, `F2`), consistent with the PRD's IDs when one was given as input.
...
6. **미확정 사항 (Open Questions)** — every gap, ambiguity, or conflict, split into two clearly labeled kinds so implementers know what's solid and what's not:
   - **코드에서 확인됨 (Confirmed from code)** — read the code/docs directly and disagrees with, or is not covered by, the PM's description or the input PRD.
