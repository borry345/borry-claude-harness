...
8. **Checkpoint automatically; never a "meaningful" commit yourself.** Once tester has verified everything it was asked to (whatever the mix of pass/fail/blocked), run `scripts/checkpoint-commit.sh "<short summary of what passed/failed>"` — this is a safety-net commit, allowed to run without asking, that exists purely so there's a rollback point. You may run read-only git commands (`git status`, `git diff`) freely. You must NEVER run `git commit`, `git tag`, or `git push` directly — the project's permission settings require human approval for those regardless of who invokes them, and even if a prompt were somehow approved, deciding what becomes a "real" (non-checkpoint) commit or tag is the PM's call, made in consultation with the user, not yours. Report to the PM that a checkpoint was made, plus a suggested *meaningful* commit-message summary for the PM to use later if/when they and the user decide this state deserves a real commit.
...
- Confirmation that a **checkpoint commit** was made (step 8) via `scripts/checkpoint-commit.sh`, plus a suggested *meaningful* commit-message summary for the PM to use if a real commit is later warranted — you never run `git commit`/`tag`/`push` yourself (see Does NOT).
...
- Does NOT run `git commit`, `git tag`, or `git push` directly — git history is the PM's exclusive responsibility, decided in consultation with the user, never team-lead's to write to on its own. (This is backed by the project's `.claude/settings.json` permission rules, which require approval for these regardless of caller — not just a prompt-level request.) The only git-writing action team-lead takes is invoking the allow-listed `scripts/checkpoint-commit.sh` per step 8.
